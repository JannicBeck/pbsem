...
#specify outer model
CB_outer_model <- list(c("SQ060","SQ062","SQ096","SQ076"),
                       c("SQ079","SQ080"),c("SQ046","SQ044"))
...
CB_scaling = list(c("NUM", "NUM","NUM", "NUM"),
                  c("NUM", "NUM"),
                  c("NUM", "NUM")
)
...
CBdata_new[ , idx] <- round(na.approx(CBdata_new[ , idx]))
CBdata_new <- CBdata_new[-261,]

#run plspm function with interpolated dataset
CB_PLS <- plspm(CBdata_new,CB_inner_model,CB_outer_model,
                modes = CB_modes, scaling = CB_scaling)
...
%                         Performance, Kundenzufriedenheit)
% 
% innerplot(CB_inner_model, main="valid, but does not work")
% 
% #works
% Preis_Leistung <- c(0,0,0)
% Kundenzufriedenheit <- c(1,0,0)
...
% 
% innerplot(CB_inner_model, main="works")
% @
...
% # <<semPLS, results='hide', warning=FALSE,message=FALSE>>=
...
% # 
% # # alternative: einlesen der splsm datei (mit Hilfe der XML Spezifikation von SmartPLS)
% # #model <- read.splsm(file=PBsmartPath)
% # 
% # #create measuring model
% # #lazy alternative instead of 5 times Preis_Leistung: rep("Preis_Leistung",5)
% # latentvar <- c("Preis_Leistung","Preis_Leistung","Preis_Leistung","Preis_Leistung","Preis_Leistung",
...
% # x <- c("Preis_Leistung","Performance")                      
% # y <- c("Kundenzufriedenheit","Kundenzufriedenheit")                         
% # CB_inner_model <- matrix(c(x,y),length(x))
% # colnames(CB_inner_model) <- c("source","target")
% # 
% # #create plsm object 
% # model <- plsm(data = CBdata, strucmod = CB_inner_model, measuremod = CB_outer_model)
% # 
...
% # 
% # PBdot <- "/home/jannic/PB-structure.dot"
...
\subsubsection{knitr}
Knitr ist ein Paket zu Erstellung von dynamischen Berichten in R mit Hilfe von Latex oder wahlweise auch Markdown. Der Vorteil davon ist, dass der R Code zum einen automatisch mit Syntaxhervorhebung gekennzeichnet wird und zum anderen dass die Ergebnisse nicht zwischengespeichert und manuell in ein anderes Dokument eingefügt werden müssen. Falls zum Beispiel eine Änderung in einem Plot vorgenommen werden muss, wie eine falsche Beschriftung einer X-Achse zu korrigieren, muss dies nur im R Code geändert werden und die Datei kompiliert werden statt den Plot neu speichern und einfügen zu müssen. Außerdem sind der Bericht und Code in einem Dokument, was ebenfalls sehr hilfreich ist.\\
Zunächst wird das Paket installiert.
<<eval=FALSE>>=
install.packages("knitr")
install.packages("xtable")
@
Anschließend muss noch in R unter Tools -> Global Options -> Sweave -> Weave Rnw files using: auf knitr gesetzt werden. Nun kann unter File -> New File -> R Sweave ein neues Projekt erstellt werden. In diesem kann dann LaTeX Code geschrieben werden und mit der folgenden Syntax auch R Code eingebettet werden. Ein R Code Block wird mit $<<>>=$ begonnen und \@ geschlossen. In den Anfangstag $<<>>=$ können weitere Parameter übergeben, welche den Output beeinflussen. Ohne Parameter wird der R Code mit Syntaxhervorhebung dargestellt, ausgeführt und die Ergebnisse wie Console Output und Plots ebenfalls in die PDF geschrieben. Dies geschieht mit dem Button "Compile PDF". Mit dem Parameter eval=FALSE wird verhindert dass der R-Code ausgeführt wird. Dieser eignet sich zum Beispiel bei install.packages oder nicht funktionierenden Code, da der Output in diesem Fall für den Leser nicht von Relevanz ist. Der Parameter include=FALSE führt dagegen den Code aus aber die Ergebnisse dieser Ausführung werden nicht in den Bericht geschrieben. Dies ist zum Beispiel für das laden von Paketen von Vorteil.
...
@
results='asis' da die Funktion xtable direkten LaTeX Code liefert und dieser so wie er ist kompiliert werden soll
echo=FALSE in der finalen Version um den print xtable Befehl zu unterdrücken
...
% 
% #get both filepaths
% dataPath <- "/home/jannic/Schreibtisch/PB/Data/Datensatz.csv"
% modelPath <- "/home/jannic/Schreibtisch/PB/Data/Modell.splsm"
% 
% #read dataset
% mvdata <- read.csv(dataPath)
% 
% #read SmartPLS model
% model <- read.splsm(modelPath)
% 
% #show the model
% model
% 
...
% <<>>=
% #specify new model
% new_PB_model <- sempls(model = plsm(data = CBdata, strucmod = cbind(c("Preis_Leistung","Performance"),c(rep("Kundenzufriedenheit",2))), measuremod = cbind(c(rep("Preis_Leistung",3),rep("Performance",2),rep("Kundenzufriedenheit",2)),c("SQ009","SQ076","SQ092","SQ079","SQ080","SQ046","SQ110"))),data = CBdata)
% 
% #show old model
% PB_model
% 
...
% 
% #execute algorithm
% new_PB_model_boot <- bootsempls(new_PB_model, nboot=5000, verbose=TRUE)
% 
% #show results
% new_PB_model_boot
% 
...
% <<>>=
% #specify new model
% diff_PB_model <- sempls(model = plsm(data = CBdata, strucmod = cbind(c("Preis_Leistung","Performance","Ambiente","Kenntnisstand"),c(rep("Kundenzufriedenheit",4))), measuremod = cbind(c(rep("Preis_Leistung",3),rep("Performance",2),rep("Kundenzufriedenheit",2),"Ambiente","Kenntnisstand"),c("SQ009","SQ076","SQ092","SQ079","SQ080","SQ046","SQ110","SQ021","SQ024"))),data = CBdata)
% 
...
\begin{figure}[h!]
\centering
\hspace*{-4.8cm}\includegraphics[scale = 0.8]{Grafiken/ausgangsmodell}
...
\begin{figure}[h!]
\centering
\hspace*{-4.8cm}\includegraphics[scale = 0.8]{Grafiken/erstesmodell}
\caption{Altes Modell CB EMPF 15}
\label{erstesmodell}
\end{figure}
...
\hspace*{-4.8cm}\includegraphics[scale = 0.882]{Grafiken/zweitesmodell}
...
\hspace*{-4.8cm}\includegraphics[scale = 0.83]{Grafiken/finalesmodell}
