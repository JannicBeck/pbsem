...
Um eine gewisse Sicherheit der Ergebnisse zu erhalten, sollen diese nachfolgend in SmartPLS und R verglichen werden. Dazu wurde das bereits in den vorherigen Kapiteln vorgestellte Modell in SmartPLS berechnet. Zu beachten sind hier die gleichen Einstellungen bei fehlenden Werten und dem Weighting Scheme. Diese wurden in SmartPLS auf "Case Replacement" und "centroid" gesetzt, um denen in R zu entsprechen. Dies ist nicht notwendig, da es auch in R durch den Parameter wscheme="pathWeighting" oder wscheme="factorial"  umgestellt werden kann, wichtig ist nur in beiden Programmen das gleiche zu verwenden. Zunächst wird noch einmal das Modell in R geplotted um diese direkt vergleichen zu können. Anschließend wird mit der rSquared() Funktion noch das $R^{2}$, welches in SmartPLS in der latenten Variable "Kundenzufriedenheit" steht, in die Console geschrieben. Dieses wird jedoch noch ausführlicher im nächsten Kapitel erläutert, hier sollen nur die Differenz der Werte interessieren.

<<>>=
#plot the R model
plot(PBcom, main="Path model in R")

#show 
rSquared(PB_model)
@

\includegraphics{Grafiken/PB_model_smartpls}
...
#execute algorithm
new_PB_model_boot <- bootsempls(new_PB_model, nboot=200, verbose=FALSE)
...
pl_fSquared <-   (rSquared(new_PB_model)[3]-rSquared(excl_pl_PB_model)[2])/(1-rSquared(new_PB_model)[3])
...
perf_fSquared <- (rSquared(new_PB_model)[3]-rSquared(excl_perf_PB_model)[2])/(1-rSquared(new_PB_model)[3])
